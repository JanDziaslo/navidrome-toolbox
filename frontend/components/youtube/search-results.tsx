      ...
      {results.map((video) => (
        ...
        >
          <CardContent className="p-3 sm:p-4 flex flex-col sm:flex-row gap-3 sm:gap-4">
            {/* Thumbnail */}
            <div className="relative w-full sm:w-40 h-48 sm:h-24 shrink-0 rounded-lg overflow-hidden bg-black">
              <Image
                src={video.thumbnail}
                alt={video.title}
                fill
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-sm sm:text-base text-text-primary font-medium line-clamp-2 mb-2 group-hover:text-accent transition-colors">
                {video.title}
              </h3>
